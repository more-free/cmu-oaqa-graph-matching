...
public class ExactMatching<V, E> extends Matching<V, E> {
    private ExactMatchingState<V, E> state;

    public ExactMatching(Graph<V, E> pattern, Graph<V, E> target) {
        super(pattern, target);
        state = new ExactMatchingState<V, E>(pattern, target);
    }
    ...
    public List<Map<V, V>> match() {
        ...
        for(V targetVertex : target.getVertices()) {
            if(targetVertex.equals(patternVertex)) {
